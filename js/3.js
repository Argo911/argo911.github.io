function Game() {
	...
	this.next = function() {
		if (!p.human && p.money < 0) {
			...
			if (p.money < 0) {
				popup("<p>" + p.name + " является банкротом. Все его активы будут переданы " + player[p.creditor].name + ".</p>", game.bankruptcy);
			} else {
				roll();
			}
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert(p.name + " купил " + sq.name + " для $" + highestbid + ".");
		}
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Аукцион <span id='propertyname'></span></div><div>Высшая ставка = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, ваша очередь делать ставки.</div<div><input id='bid' title='Enter an amount to bid on " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='поставить' onclick='game.auctionBid();' title='Place your bid.' /><input type='button' value='пас' title='Skip bidding this time.' onclick='game.auctionPass();' /><input type='button' value='Выйти из аукциона' title='Stop bidding on " + s.name + " altogether.' onclick='if (confirm(\"Вы уверены, что хотите полностью прекратить участие в торгах по этому объекту?\")) game.auctionExit();' /></div>", "blank");
...
//***Отвечает за показание карточеи собственности в аукционе***//
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						window.alert(p.name + " вышел из аукциона.");
						continue;

					} else if (bid === 0) {
						window.alert(p.name + " пропустил.");
						continue;

					} else if (bid > 0) {
						this.auctionBid(bid);
						window.alert(p.name + " делать ставку $" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			...
			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "У вас недостаточно денег, чтобы сделать ставку $" + bid + ".";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Ваша ставка должна быть выше самой высокой ставки. $" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};
	...
	var tradeMoneyOnChange = function(e) {
		...
		if (amount < 0) {
			this.value = "Это значение должно быть больше 0.";
			this.style.color = "red";
			return false;
		}
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		if (initiator.communityChestJailCard) {
			...
			currentTableCellCheckbox.title = "Установите этот флажок, чтобы включить эту бесплатную карту (Выйти из тюрьмы) в обмен.";
			...
			currentTableCell.textContent = "Бесплатная карта «Выйти из тюрьмы»";
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.title = "Выберите игрока, с которым хотите торговать.";
		} else {
	...
	this.acceptTrade = function(tradeObj) {
		...
		if (money > 0 && money > initiator.money) {
			document.getElementById("trade-leftp-money").value = initiator.name + " не имеет $" + money + ".";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		} else if (money < 0 && -money > recipient.money) {
			document.getElementById("trade-rightp-money").value = recipient.name + " не имеет $" + (-money) + ".";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		if (showAlerts && !confirm(initiator.name + ", вы уверены, что хотите совершить этот обмен с " + recipient.name + "?")) {
			return false;
		}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (tradeObj.getProperty(i) === 1) {
				...
				addAlert(recipient.name + " получите " + square[i].name + " от " + initiator.name + ".");
			} else if (tradeObj.getProperty(i) === -1) {
				square[i].owner = initiator.index;
				addAlert(initiator.name + " получите " + square[i].name + " от " + recipient.name + ".");
			}
		...
		if (money > 0) {
			...
			addAlert(recipient.name + " получено $" + money + " от " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert(initiator.name + " получено $" + money + " от " + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		if (isAPropertySelected === 0) {
			popup("<p>Для обмена необходимо выбрать одно или несколько собственностей.</p>");
		...
		if (recipient.human) {
			...
			addAlert(initiator.name + " инициировал сделку с " + recipient.name + ".");
			popup("<p>" + initiator.name + " предложил обмен с вами, " + recipient.name + ". Вы можете принять, отклонить или изменить предложение.</p>");
		} else {
			...
			if (tradeResponse === true) {
				...
				popup("<p>" + recipient.name + " сделал встречное предложение.</p>");
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup("<p>Поздравляю, " + player[1].name + ", ты выиграл игру.</p><div>");
	...
	this.resign = function() {
		popup("<p>Вы уверены, что хотите сдаться</p>", game.bankruptcy, "Yes/No");
	};
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert(pcredit.name + " оплатите $" + bankruptcyUnmortgageFee + " проценты за заложенное имущество, полученные от " + p.name + ".");
			popup("<p>" + pcredit.name + ", ты должен заплатить $" + bankruptcyUnmortgageFee + " проценты на заложенное имущество, которое вы получили от " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function updateOwned() {
	...
	if (p.communityChestJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Бесплатная карта «Выйти из тюрьмы»</td></tr>";
	...
	if (p.chanceJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Бесплатная карта «Выйти из тюрьмы»</td></tr>";
	}

	if (HTML === "") {
		HTML = p.name + ", у вас нет никакой собственности.";
		$("#option").hide();
	} else {
		HTML += "</table>";
	}
...
function updateOption() {
	...
	if (sq.mortgage) {
		document.getElementById("mortgagebutton").value = "Выкупить ($" + Math.round(sq.price * 0.55) + ")";
		document.getElementById("mortgagebutton").title = "Выкупить " + sq.name + " за $" + Math.round(sq.price * 0.55) + ".";
		$("#buyhousebutton").hide();
		$("#sellhousebutton").hide();

		allGroupUnmortgaged = false;
	} else {
		document.getElementById("mortgagebutton").value = "заложить ($" + (sq.price * 0.5) + ")";
		document.getElementById("mortgagebutton").title = "заложить " + sq.name + " за $" + (sq.price * 0.5) + ".";
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Купить дом ($" + sq.houseprice + ")";
			sellhousebutton.value = "Продать дом ($" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Купить дом за $" + sq.houseprice;
			sellhousebutton.title = "Продать дом за $" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Купить отель ($" + sq.houseprice + ")";
				buyhousebutton.title = "Купить отель за $" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
			...
			for (var i = 0; i < max; i++) {
				...
				if (s.owner !== sq.owner) {
					...
					buyhousebutton.title = "Прежде чем вы сможете купить дом, вы должны владеть всей собственностью этой цветовой группы.";
				} else {
			...
			if (sq.house > minhouse) {
				...
				if (sq.house == 1) {
					buyhousebutton.title = "Прежде чем вы сможете купить еще один дом, вся собственность этой цветовой группы должны иметь один дом.";
				} else if (sq.house == 4) {
					buyhousebutton.title = "Прежде чем вы сможете купить отель, вся остальная собственность этой цветовой группы должна иметь 4 дома.";
				} else {
					buyhousebutton.title = "Прежде чем вы сможете купить дом, вся остальная собственность этой цветовой группы должны быть" + sq.house + " дома.";
				}
			}
			if (sq.house < maxhouse) {
				...
				if (sq.house == 1) {
					sellhousebutton.title = "Прежде чем вы сможете продать дом, вся остальная собственность этой цветовой группы должна иметь один дом.";
				} else {
					sellhousebutton.title = "Прежде чем вы сможете продать дом, вся остальная собственность этой цветовой группы должна быть " + sq.house + " дома.";
				}
			}
			...
			if (!allGroupUninproved) {
				document.getElementById("mortgagebutton").title = "Прежде чем имущество может быть заложено, вся собственность его цветовой группы не должны быть улучшены.";
...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		popup("<img src='images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Общественный фонд:</div><div style='text-align: justify;'>" + communityChestCards[communityChestIndex].text + "</div>", function() {
...
function addamount(amount, cause) {
	...
	addAlert(p.name + " received $" + amount + " от " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert(p.name + " потерял $" + amount + " из-за " + cause + ".");
}
...
function payeachplayer(amount, cause) {
	...
	addAlert(p.name + " потерял $" + total + " от " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert(p.name + " получите $" + total + " из " + cause + ".");
}
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			addAlert(p.name + " получите зарплату в размере $200 за прохождение GO.");
		}
	}
	if (p.position < destination) {
		...
		p.position = destination;
		p.money += 200;
		addAlert(p.name + " получил зарплату в размере $200 за прохождение GO.");
	}
...
function streetrepairs(houseprice, hotelprice) {
	...
	if (cost > 0) {
		...
		if (houseprice === 40) {
			addAlert(p.name + " отдай $" + cost + " в сундук сообщества.");
		} else {
			addAlert(p.name + " отдай $" + cost + " к шансу.");
		}
	}
...
function payfifty() {
	...
	addAlert(p.name + "заплатил штраф в размере 50$, чтобы выйти из тюрьмы");
	updateMoney();
	updatePosition();
}
	...
	for (var x = 1; x <= pcount; x++) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (sq.owner == x) {
				...
				if (sq.mortgage) {
					mortgagetext = "title='Заложить' style='color: grey;'";
				}
		...
		if (p.communityChestJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Бесплатная карта Выйти из тюрьмы</td></tr>";
		...
		if (!write) {
			HTML += p.name + " не имеет имущества.";
		} else {
			HTML += "</table>";
		}
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert(p.name + " купил " + property.name + " за " + property.pricetext + ".");
		...
		popup("<p>" + p.name + ", тебе нужно на  $" + (property.price - p.money) + " больше чтобы купить " + property.name + ".</p>");
	}
}
...
function mortgage(index) {
	...
	addAlert(p.name + " заложенный " + sq.name + " для $" + mortgagePrice + ".");
	updateOwned();
	updateMoney();
...
function land(increasedRent) {
	...
	$("#landed").show();
	document.getElementById("landed").innerHTML = "Вы попали на " + s.name + ".";
	s.landcount++;
	addAlert(p.name + " попал на " + s.name + ".");
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>Вы попали на <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Купить ($" + s.price + ")' title='Купить " + s.name + " для " + s.pricetext + ".'/></div>";
		}
		...
		if (p.position == 5 || p.position == 15 || p.position == 25 || p.position == 35) {
			...
			if (!groupowned) {
				...
				if (s.house === 0) {
					...
					rent = s["аренда" + s.house];
				}
			}
		}

		addAlert(p.name + " платный $" + rent + " аренда до " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "Вы высадились на " + s.name + ". " + player[s.owner].name + " собери $" + rent + " аренду.";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Вы высадились на " + s.name + ". Имущество заложено; арендная плата не взималась.";
	}
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Отправляйтесь в тюрьму. Идите прямо в тюрьму. Не проходите GO. Не собирайте 200 долларов.</div>", gotojail);
		} else {
			gotojail();
		}
...
function roll() {
	...
	document.getElementById("nextbutton").value = "Закончить ход";
	document.getElementById("nextbutton").title = "Завершите ход и перейдите к следующему игроку.";
	...
	if (die1 == die2) {
		addAlert(p.name + " выпало " + (die1 + die2) + " - дубль.");
	} else {
		addAlert(p.name + " выпало " + (die1 + die2) + ".");
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "бросить снова";
			document.getElementById("nextbutton").title = "Вы бросили дубль. Бросай снова.";

		} else if (doublecount === 3) {
			p.jail = true;
			doublecount = 0;
			addAlert(p.name + "выпадал дубль три раза подряд.");
			updateMoney();


			if (p.human) {
				popup("Вы три раза подряд выпали двойки. Отправляйтесь в тюрьму.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>Вы должны заплатить штраф в размере 50 долларов.</p>", function() {
						payfifty();
		...
		if (p.position >= 40) {
			p.position -= 40;
			p.money += 200;
			addAlert(p.name + " получил зарплату в 200 долларов за прохождение GO.");
		}
...
function play() {
	...
	document.getElementById("nextbutton").title = "Бросьте кубики и переместите свой жетон соответствующим образом.";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Вы в тюрьме.<input type='button' title='Заплатить $50 штраф, чтобы немедленно выйти из тюрьмы.' value='Pay $50 fine' onclick='payfifty();' />";
		...
		document.getElementById("nextbutton").title = "Бросайте кости. Если вы выбросите двойное число, вы выйдете из тюрьмы.";

		if (p.jailroll === 0)
			addAlert("This is " + p.name + "первый поворот в тюрьме.");
		else if (p.jailroll === 1)
			addAlert("This is " + p.name + "второй ход в тюрьме.");
			...
			addAlert("This is " + p.name + " третий ход в тюрьме.");
		}
...
window.onload = function() {
	...
	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Тюрьма</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";
	...
	$("#mortgagebutton").click(function() {
		...
		if (s.mortgage) {
			if (player[s.owner].money < Math.round(s.price * 0.55)) {
				popup("<p>Вам нужно $" + (Math.round(s.price * 0.55) - player[s.owner].money) + "больше, чтобы снять ипотеку " + s.name + ".</p>");

			} else {
				popup("<p>" + player[s.owner].name + ", ты уверен, что хочешь выкупить" + s.name + " за $" + Math.round(s.price * 0.55) + "?</p>", function() {
					unmortgage(checkedProperty);
				}, "Yes/No");
			}
		} else {
			popup("<p>" + player[s.owner].name + ",вы уверены, что хотите заложить " + s.name + " за $" + Math.round(s.price * 0.5) + "?</p>", function() {
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>Вам нужно $" + (s.houseprice - player[s.owner].money) + " больше, чтобы купить отель для " + s.name + ".</p>");
				return;
			} else {
				popup("<p>Вам нужно $" + (s.houseprice - player[s.owner].money) + " больше, чтобы купить дом за " + s.name + ".</p>");
				return;
			}
		}
		...
		if (s.house < 4 && houseSum >= 32) {
			popup("<p>Все 32 дома находятся в собственности. Вы должны подождать, пока один из них освободится.</p>");
			return;
		} else if (s.house === 4 && hotelSum >= 12) {
			popup("<p>Все 12 отелей находятся в собственности. Вы должны подождать, пока один из них освободится.</p>");
			return;
		}
...
function soundClick() {
	...
	q=['mp3/Sabaton_-_Primo_Victoria_(musmore.com).mp3','mp3/Sabaton_-_Panzerkampf_(musmore.com).mp3','mp3/Sabaton_-_Night_Witches_(musmore.com).mp3','mp3/Blur_-_Song_2_(musmore.com).mp3','mp3/dvrst-igor-sklyar-atomic-heart-komarovo-dvrst-phonk-remix-mp3.mp3'];
	audio.src = q[a];
	audio.loop = true;
